...
async function showPage(page, content = '') {
    ...
    // اگر کاربر دسترسی دارد، چت را با تاخیر راه‌اندازی کن
    if (state.user && state.user.canChat) {
        console.log('⏳ Setting up chat with delay...');
        // ابتدا مطمئن شویم Socket متصل است
        ...
        setTimeout(() => {
            setupChat();
            // درخواست مجدد تاریخچه بعد از 1 ثانیه
            setTimeout(() => {
                if (state.socket && state.socket.connected) {
