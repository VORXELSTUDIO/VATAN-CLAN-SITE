...
// ============================================
// CHAT SYSTEM (نسخه نهایی با مدیریت حذف کاربر)
// ============================================
function setupSocket() {
    ...
    // ثبت شناسه کاربر در سرور با تاخیر کم
    ...
    state.socket.on('connect', () => {
        ...
        // دوباره ثبت کاربر هنگام اتصال مجدد
        if (state.user && state.user.id) {
            state.socket.emit('registerUser', state.user.id);
        }
    });
    ...
    // ============================================
    // رویداد خروج اجباری (وقتی کاربر حذف می‌شود)
    // ============================================
    ...
    // ============================================
    // رویداد حذف کاربر (برای اطلاع سایر کاربران)
    // ============================================
    state.socket.on('userDeleted', (data) => {
        console.log(`🗑️ User deleted: ${data.username} (${data.userId})`);
        // اگر لیست کاربران باز است، آن را به‌روزرسانی کنیم
