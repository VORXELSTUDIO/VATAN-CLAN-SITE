...
async function readJSON(filename) {
    ...
    try {
        ...
        // اگه JSON خراب بود، فایل رو ریست کن
...
async function writeJSON(filename, data) {
    ...
    try {
        // پشتیبان گرفتن از فایل قدیمی
        const backupPath = filePath + '.bak';
        try {
            await fs.copyFile(filePath, backupPath);
        } catch (e) {
            // فایل اصلی وجود نداره، اشکالی نداره
        }
        
        // نوشتن توی فایل موقت (atomic write)
...
io.on('connection', (socket) => {
    ...
    socket.on('game-over', (data) => {
        if (currentRoom) {
            ...
            // ذخیره تاریخچه با try-catch (خطا رو نادیده بگیر)
            try {
                saveGameHistory({
                    id: generateId(),
                    roomId: currentRoom,
