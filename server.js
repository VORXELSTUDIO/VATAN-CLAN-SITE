...
const ChatMessage = {
    ...
    create: (data) => {
        const db = readDB();
        const newItem = { _id: generateId(), ...data, timestamp: new Date().toISOString() };
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        // بررسی اینکه اسم نمایشی فقط انگلیسی باشه (سمت سرور)
...
app.put('/api/users/:id/chat', auth, adminOnly, async (req, res) => {
    ...
    const user = await User.findByIdAndUpdate(req.params.id, { canChat });
...
app.put('/api/users/:id/make-admin', auth, adminOnly, async (req, res) => {
    ...
    try {
        const db = readDB();
        console.log('📊 Database users:', db.users.length);
...
app.delete('/api/users/:id', auth, adminOnly, async (req, res) => {
    try {
        ...
        // جلوگیری از حذف مدیر سایت (ادمین اصلی)
