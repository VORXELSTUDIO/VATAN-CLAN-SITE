...
// ============================================
// قفل برای فایل‌ها (جلوگیری از خرابی JSON)
// ============================================
...
// ============================================
// Database Functions (با قفل و پشتیبان)
// ============================================
...
async function readJSON(filename) {
    ...
    try {
        ...
        if (error instanceof SyntaxError) {
            console.error(`❌ ${filename} is corrupted! Resetting to empty array...`);
...
// ====== اینجا اصلاح شد: isAdmin برداشته شد ======
app.post('/api/users/coins', authenticateToken, async (req, res) => {
    try {
        const { userId, amount } = req.body;
        
        console.log(`💰 Updating coins: userId=${userId}, amount=${amount}`);
        ...
        console.log(`✅ Coins updated: ${user.displayName} now has ${user.coins} coins`);
        ...
        res.status(500).json({ error: 'خطا در به‌روزرسانی سکه: ' + error.message });
    }
});
    ...
    socket.on('game-over', (data) => {
        if (currentRoom) {
            ...
            try {
                ...
                }).catch(err => console.log('⚠️ Could not save game history:', err.message));
            } catch (error) {
                console.log('⚠️ Could not save game history:', error.message);
            }
    ...
    try {
        ...
        if (shopItems.length === 0) {
            const items = [
                ...
                { id: 'avatar3', type: 'avatar', name: 'شیر', price: 60, icon: '🦁' },
                ...
                { id: 'avatar8', type: 'avatar', name: 'فروهر', price: 100, icon: '🦅' },
